###########################################################
# Entire Cassiope analysis - part 8c - Demography
# March 2023
# R fastsimcoal2 plotting demographic scenarios - set values line 106
#####################################
# copied from: http://cmpg.unibe.ch/software/fastsimcoal2/parFileViewer.html
...
propLastsegment=0.05                        #Percentage of page to keep on y axis above last event 
...
if (!is.na(args[1]) & length(args)) { #Fix some parameters when script called from outside
  ...
  externalCall=T        #Just to say the program has been called from outside
   ...
   setwd("~/GitHub/Rieseberg_Lab/Population_genomics_Cassiope/Figures_data/fastsimcoal2")
   
   # parFileName= "5pops_mertensiana_maxL.par"
   # popLabels=c("Russia", "Alaska", "Europe","BCmer", "BCtet")
   # usePopLabels=T         #Use popLabels on x axis, if "F" then plots deme numbers 0 1 2 3 etc...
   # genTime=10              #Arbitrary generation time (if genTime !=1 then time is given in years)
   # fixMaxTime=T           #Allow one to plot only a given time scale up to maxTimeToPlot
   # maxTimeToPlot=18700000    #Upper limit for time scale if fixMaxTime==T
   # 
   # parFileName= "6pops_mertensiana_maxL.par"
   # popLabels=c("Russia", "Alaska", "Europe","BCmer", "Greenland")
   # usePopLabels=T         #Use popLabels on x axis, if "F" then plots deme numbers 0 1 2 3 etc...
   # genTime=10              #Arbitrary generation time (if genTime !=1 then time is given in years)
   # fixMaxTime=T           #Allow one to plot only a given time scale up to maxTimeToPlot
   # maxTimeToPlot=18700000     #Upper limit for time scale if fixMaxTime==T

   # parFileName= "5pops_tet_maxL.par"
   # popLabels=c("Russia", "Alaska", "Europe","NWT", "Nunavut")
   # usePopLabels=T         #Use popLabels on x axis, if "F" then plots deme numbers 0 1 2 3 etc...
   # genTime=10              #Arbitrary generation time (if genTime !=1 then time is given in years)
   # fixMaxTime=T           #Allow one to plot only a given time scale up to maxTimeToPlot
   # maxTimeToPlot=1280000     #Upper limit for time scale if fixMaxTime==T

   parFileName= "6pops_tet_maxL.par"
   popLabels=c("Greenland", "Alaska", "Europe","NWT", "Nunavut")
   usePopLabels=T         #Use popLabels on x axis, if "F" then plots deme numbers 0 1 2 3 etc...
   genTime=10              #Arbitrary generation time (if genTime !=1 then time is given in years)
   fixMaxTime=T           #Allow one to plot only a given time scale up to maxTimeToPlot
   maxTimeToPlot=1280000     #Upper limit for time scale if fixMaxTime==T
