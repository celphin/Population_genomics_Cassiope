...
setwd("~/GitHub/Population_genomics_Cassiope/Mimulus/Timeseries")

#Load the gds file - for PCA
genofile <- snpgdsOpen("./Figures_data/Mimulus_filtered_rm20.recode.gds")

# load list of samples in filtered vcf
samples_list <- read.table("./Figures_data/Mimulus_filtered_rm20.imiss", header = TRUE)

# load in Admixture data - code setup for 5 populations
Admix_tbl=read.table("./Figures_data/Admixture/Take1/Mimulus_filtered_rm20.4.Q")
...
# Population lat long
pop_lat_long <- read.csv("./Figures_data/Lat_long_Coord.csv", header = TRUE)
...
round(pc.percent, 2)
# 46.52 10.27  5.69  4.44  3.42  2.97  2.36  2.12  1.96  1.91  1.37  1.27  1.25  1.23  1.20  1.18
...
All_pop_data_map <- left_join(pop_lat_long, Pop_avg, by="Site")
All_pop_data <- Pop_avg
...
All_pop_data  <- transform_coord(All_pop_data_map, lon = "Long", lat = "Lat", bind = TRUE, proj.og = "+proj=longlat +datum=WGS84", proj.out = "+init=epsg:3857")
